...
async def gintoki(ctx):
    ...
    embed_toki.set_thumbnail(
        url='https://cdn.discordapp.com/avatars/830328082724290611/bc40e3432b248b159c52e5da340ff710.webp?size=512')
...
async def seggs(ctx, member: discord.Member = None, member1: discord.Member = None, member2: discord.Member = None):
    ...
    if percent < 30:
        segs = discord.Embed(title=f'\n{name}  💏  {name1}  💏  {name2}',
                             description=f'**Seggs Quality**: {percent}%  😕\n\n**Seggs Alias**: {namex}{namew}{namey}',
...
async def invite(ctx):
    await ctx.reply(
        'use this link to add this shitty bot 😎🙏\nhttps://discord.com/api/oauth2/authorize?client_id=944235413499945000&permissions=1274666151121&scope=bot')


@client.command()
async def winter(ctx):
    await ctx.reply(
        'https://cdn.discordapp.com/attachments/858743303252344845/950456155677818890/GIF-220307_234312.gif')
...
async def segs(ctx, member: discord.Member = None, member1: discord.Member = None, ):
    ...
    if percent < 30:
        segs = discord.Embed(title=f'\n{name}  💏  {name1}',
                             description=f'**Segs Quality**: {percent}%  😕\n\n**Segs Alias**: {namex}{namey}',
                             color=ctx.author.color)
        segs.set_author(name=f'{ctx.author.name}\'s Segs Chamber', icon_url=ctx.author.avatar_url)
    elif 69 > percent > 30:
        segs = discord.Embed(title=f'\n{name}  💏  {name1}',
                             description=f'**Sggs Quality**: {percent}%  😃\n\n**Segs Alias**: {namex}{namey}',
                             color=ctx.author.color)
        segs.set_author(name=f'{ctx.author.name}\'s Segs Chamber', icon_url=ctx.author.avatar_url)
    elif percent == 69:
        segs = discord.Embed(title=f'\n{name}  💏  {name1}',
...
async def pray(ctx, member: discord.Member = None):
    ...
    if not member or member == ctx.author:
        ...
        await ctx.reply(format(
            ctx.author.mention) + ' Prays For ' + member.mention + ' <:AyaPray:916508556839256155> <:prayge:915560131612258314>')
...
async def gay(ctx, member: discord.Member = None):
    ...
    embed = discord.Embed(title=format(member.display_name) + "'s " 'Gay percentage', colour=member.colour,
                          description=format(
                              member.display_name) + ' is ' + gay + '%' + ' Gay <:kokoOnegai:916355927219798016>')
...
async def pp(ctx, member: discord.Member = None):
    ...
    if member.id == 434254619842183168:
        n = 9
    embed1 = discord.Embed(title=format(member.display_name) + "'s " 'pp size', colour=member.colour,
                           description='8' + (length * n) + 'D')
    ...
    embed2 = discord.Embed(title=format(member.display_name) + "'s " 'pp size', colour=member.colour,
                           description='8' + (length * n) + 'D')
    embed2.set_thumbnail(
        url='https://preview.redd.it/afp0ri5iq3u51.png?auto=webp&s=1f4cbdac2563cfa9526a62b0e77331ff140b97c2')
    embed2.set_footer(text=f"Requested by: {ctx.author}", icon_url=ctx.author.avatar_url)
    if n <= 4:
        await ctx.reply(embed=embed2)
...
async def reminder(ctx, time, *, reminder):
    ...
    if seconds == 0:
        ...
        remind = discord.Embed(title='Reminder Set', colour=user.colour,
                               description=f"will remind you about {reminder} in {counter} , <:sedlyf:909883405108412447>")
...
async def help(ctx, message=''):
    ...
    page2.add_field(name='`hasde  `',
                    value='Hasde 😃chehreya 🙂da matlab ❓eh nhi 🚫hunda ki ohnanu koi 😔takleef 😖nhi hundi',
                    inline=False)
...
async def balance(ctx, user: discord.Member = None):
    ...
    bal = discord.Embed(title=f"{user.name}'s GII balance ", colour=0x000080,
                        description='**Wallet**: ' + '<:gems:919294512847786045> ' + str(
                            wallet_amt) + '   ,  **Bank**:  ' + '<:gems:919294512847786045> ' + str(bank_amt),
                        timestamp=ctx.message.created_at)
    bal.set_footer(text=f"Requested by: {ctx.author}", icon_url=ctx.author.avatar_url)
    bal.set_thumbnail(
        url='https://cdn.discordapp.com/attachments/858743303252344845/980106603707637840/FTh_5FxXoAIkxw_.jpg')
...
async def beg(ctx):
    ...
    beg_list = ['SamuRay', 'Griffin', 'YeXiU', 'Drake', 'Falme', 'Varka', 'Frosty', 'Crunchy', 'Kr0n0s', 'SiCK',
                'Gintoki', 'Akagami', 'ITACHY', 'Matrix', 'Lordiee']

    response = ['true', 'false']

    beg_choice = random.choice(beg_list)
    response_res = random.choice(response)
    refuse = ['said : **GN**', 'said : *Go away you peasant*', '*Whined like a cow and refused*',
              'said : *Go beg somewhere else*', 'said : *Kal Ana*', 'said : <:QiQiDisgust:851307847185989642>',
              'said : <:TaoReally:945658343022792715>']
...
async def slots(ctx, amount=None):
    ...
    if slot1 == slot2 == slot3:
        ...
        em18 = discord.Embed(title=f"Slots Machine for {ctx.author}", color=discord.Color.green(),
                             timestamp=ctx.message.created_at)
        em18.add_field(name=slotOutput.format(slotOutput),
                       value=f"You quadrupled your **{amount}** Primos!:moneybag: :moneybag: ")
        await ctx.send(embed=em18)
        return

    elif slot1 == slot2 or slot1 == slot3 or slot2 == slot3:
        await update_bank(ctx.author, 2 * amount)
        em17 = discord.Embed(title=f"Slots Machine for {ctx.author} ", color=discord.Color.green(),
                             timestamp=ctx.message.created_at)
        em17.add_field(name=slotOutput.format(slotOutput),
                       value=f"You doubled your **{amount}** Primos!:money_with_wings: :money_with_wings: ")
        ...
        em19.add_field(name=slotOutput.format(slotOutput),
                       value=f"You lost **{amount}** poros!**\n **Better luck next time.")
    ...
    for i in range(len(list)):
        if message.content.lower().startswith(f'l-{list[i]}') or message.content.lower().startswith(f'L-{list[i]}'):
            ...
            if not message.mentions:
                ...
                cat.set_author(name=f'{str(message.author)[:-5]} {choice}-s  {name[:-5]} 🤠 ',
